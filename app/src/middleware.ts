...
export default auth((req) => {
  ...
  const isInstrutorProtected = pathname.startsWith('/instrutor') && pathname !== '/instrutor/login';
  ...
  if (!isLoggedIn) {
    ...
    if (isInstrutorProtected) return Response.redirect(new URL('/instrutor/login', req.url));
    if (isAdminProtected) return Response.redirect(new URL('/login', req.url));
  }
});

export const config = {
  matcher: ['/dashboard/:path*', '/organizations/:path*', '/subscriptions/:path*', '/webhook-logs/:path*', '/usuarios/:path*', '/config/:path*', '/escolinha/:path*', '/onboarding/:path*', '/instrutor/:path*'],
};
