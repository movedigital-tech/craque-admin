...
import { mensalidadeStatusMap, turmaDetalheAlunos } from '../../data/escolinha';
...
export function EscolinhaTurmaDetalhe() {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <Button variant="secondary" size="sm" leadingIcon="pencil" onClick={() => navigate('/escolinha/turmas/novo')}>
            Editar turma
          </Button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 300px', gap: 20 }}>
        <Card padding={0} style={{ overflow: 'hidden' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['Aluno', 'Responsável', 'Presença', 'Mensalidade', ''].map((h) => (
                  <th key={h} style={th}>
                    {h}
                  </th>
                ))}
