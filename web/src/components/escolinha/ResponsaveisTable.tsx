...
export function ResponsaveisTable({ responsaveis }: { responsaveis: ResponsavelRow[] }) {
  ...
  const list = responsaveis.filter((r) => chip === 'all' || (chip === 'completo' ? r.completo : !r.completo));
  ...
  return (
    <Card padding={0} style={{ overflow: 'hidden' }}>
      ...
      {list.length === 0 ? (
        <p style={{ padding: '20px 24px', margin: 0, color: 'var(--text-secondary)', fontSize: 'var(--fs-body)' }}>Nenhum responsável cadastrado ainda.</p>
      ) : (
